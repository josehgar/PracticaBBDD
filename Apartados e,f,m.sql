...
-- e)
SELECT p.name, COUNT(u.procedureid) AS number_procedures, IFNULL(SUM(m.cost), 0) AS total_costs, IFNULL(AVG(m.cost), 0) AS average_costs
FROM physician p 
...
-- m)
DELIMITER $$
CREATE FUNCTION calc_stay_cost(patient_stay_id INT)
RETURNS DECIMAL
DETERMINISTIC
BEGIN
    ...
    WHERE s.stayid = patient_stay_id;
...
SELECT calc_stay_cost(3215) AS 'Coste Estancia 3215', calc_stay_cost(3216) AS 'Coste Estancia 3216', calc_stay_cost(3217) AS 'Coste Estancia 3217';